                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <div class="card-tools">
                                <a href="{{ route('zakat-transactions.create') }}" class="btn btn-primary"><i class="fas fa-plus"></i> Tambah</a>
                            </div>
                        </div>
                        <div class="card-body">
...
<script>
    const table = $('#data_table').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: '{{ route("zakat-transactions.index") }}',
            type: 'POST',
            headers: {
        ...
        columns: [{
            ...
            },
            {
                data: null,
                render: function(data, type, row) {
                    return '<a href="{{ route("zakat-transactions.index") }}/' + data.id + '/edit" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i> Edit</a> ' +
    ...
    $('#data_table').on('click', '.btn-delete', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("zakat-transactions.index") }}/' + id,
                    type: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                    },
